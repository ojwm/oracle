...
declare
   ...
   cursor c_get_indx (c_owner in varchar2,
                      c_table in varchar2) is
      select distinct(index_name) index_name
      from   all_ind_columns
      where  table_name  = UPPER(c_table)
      and    table_owner = UPPER(c_owner);

   cursor c_get_cols (c_owner in varchar2,
                      c_index in varchar2,
                      c_table in varchar2) is
...
begin
   ...
   begin
      ...
      ctrl_run_prog := 'select count(*) from '||v_table_owner||'.'||v_table_name;
...
dbms_output.put_line('');
   ...
   FOR l_c_get_info in c_get_info (v_table_name) LOOP
      ...
      ELSIF l_c_get_info.data_type LIKE 'TIMESTAMP%' then
   ...
   FOR l_c_get_indx IN c_get_indx (v_table_owner, v_table_name) LOOP

      v_indx_name  := l_c_get_indx.index_name;
      v_row_length := 0;

      FOR l_c_get_cols in c_get_cols (v_table_owner, v_indx_name, v_table_name) LOOP
